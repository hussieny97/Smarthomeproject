...
void mspi_init(u8_t main_mode,u8_t data_mode, u8_t clk_prescaler)
{
	switch(main_mode)
	{
	case Master:
	...
	MSPI_SPCR = (0x10)/*0001 0000 */ | (data_mode << 2);//0101 1101
	break;
	}
	
	case Slave:
	{
		/*Enable slave mode, Sending MSB first and Setting data mode*/
		MSPI_SPCR = (0x00) | (data_mode << 2); //0b00000
		break;
	}
	}
	/*Check if the prescaler value is divisible by two or not*/
